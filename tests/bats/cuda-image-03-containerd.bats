...
@test "IMG-03: images/config.toml.tmpl baked at /var/lib/rancher/k3s/agent/etc/containerd/ sets default_runtime_name = \"nvidia\"" {
  run docker run --rm --entrypoint sh "$TAG" -c '
    set -eu
    template=/var/lib/rancher/k3s/agent/etc/containerd/config.toml.tmpl
    [ "$(grep -Fo "{{ template \"base\" . }}" "$template" | wc -l)" -eq 1 ]
    [ "$(grep -Fxc "{{ template \"base\" . }}" "$template")" -eq 1 ]
    grep -Fq "default_runtime_name = \"nvidia\"" "$template"
  '
  [ "$status" -eq 0 ]
}
