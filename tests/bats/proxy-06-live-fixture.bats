...
# Live leak-defense fixture verification:
# (1) git check-ignore catches the karyon-tenants/ glob — the probe path MUST be
#     REPO-RELATIVE (LOAD-BEARING: with an absolute path outside the work tree,
#     git check-ignore reports no match and the test passes for the wrong reason).
# (2) gitleaks detect fires on the checked-in positive fixture (synthetic-kubeconfig.yaml).
...
@test "PROXY-03 leak defense: gitleaks fires on tenant-shaped fixture NOT in allowlist" {
  # Use BATS_TEST_TMPDIR (auto-cleaned by bats on test exit, including crashes).
...
teardown() {
  ...
  # Defensive cleanup of any pre-fix-era orphan fixtures left behind by older
