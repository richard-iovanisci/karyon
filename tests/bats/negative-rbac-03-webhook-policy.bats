...
# Webhook-policy denial probes: namespace prefix, namespace quota, registry allowlist.
# The quota-error grep is deliberately permissive -- Capsule's denial wording varies.
# Idempotent quota precondition: delete leftover alpha-* namespaces EXCEPT
# tenant-alpha + alpha-app1. KARYON_PHASE11_STRICT_LIVE=1 converts skips to FAIL.
...
setup_file() {
  ...
  # Idempotent precondition for the quota test.
  # Delete any leftover alpha-* namespaces from prior failed runs, EXCEPT:
  #   - tenant-alpha (the home namespace)
  #   - alpha-app1   (the impersonation-test fixture; required for a starting count of 2)
  # Without this cleanup, leftover alpha-app2/app3 from prior runs cause a false-pass
...
@test "N6 -- tenant quota violation denied (real CRUD probe; Pitfall 11-P3 permissive grep)" {
  # setup_file did the precondition cleanup.
  # Skip if alpha-app1 is absent (impersonation-test fixture missing).
  if ! kubectl --context=k3d-spoke-capsule get namespace alpha-app1 >/dev/null 2>&1; then
  ...
  # Pre-state (post-cleanup): tenant-alpha (home) + alpha-app1 (fixture) = 2 namespaces
  # 3rd namespace (within quota) -- should succeed
  run kubectl --kubeconfig="$ALPHA_KUBECONFIG" create namespace alpha-app2
  [ "$status" -eq 0 ]
  # Capsule's namespaces.projectcapsule.dev
  ...
  # reconciled status.size from 2 -> 3 yet (race observed on a live run:
...
@test "N7 -- denied registry pull (Capsule containerRegistries allowlist webhook)" {
  # alpha-app1 is created by the impersonation live test. If absent (cluster reset), test skips below.
  if ! kubectl --context=k3d-spoke-capsule get namespace alpha-app1 >/dev/null 2>&1; then
    _strict_skip "alpha-app1 namespace absent (Phase 9 TEN-02 fixture missing); re-create before re-running"
  fi
  # `docker.io` was later added to alpha + bravo `containerRegistries.allowed[]`
  # (required so the seeded `docker.io/library/nginx:alpine` workload is webhook-accepted;
  # the webhook matches fully-qualified image references). This probe MUST pull from a
  # registry NOT in `allowed[]` to exercise the webhook. With
  # `allowed[] = [registry.example.io, docker.io]`, `quay.io/library/nginx` is the
  # canonical "different registry" probe — the fixture was retargeted to a registry
  # still outside the allowlist.
  run kubectl --kubeconfig="$ALPHA_KUBECONFIG" run probe-n7 \
    --image=quay.io/library/nginx -n alpha-app1
  [ "$status" -ne 0 ]
  # Probe-validity tightening:
  ...
  # the probe could otherwise pass for unrelated reasons (e.g., a `forbidden` from RBAC).
  ...
  #      NOT a Capsule webhook registry-allowlist denial. If the Capsule pods-CREATE
  #      webhook is not registered at all, this @test is expected to fail.
