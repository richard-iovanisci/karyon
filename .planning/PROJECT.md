...
- ~~Repo visibility decision~~ — **RESOLVED 2026-07-07: flipped back to PUBLIC** (design intent restored). Pre-flight ran clean: strict full-history gitleaks (allowlist stripped — only the known-inert synthetic fixtures), employer/PII sweep (zero hits incl. history), author email = GitHub identity. The per-tenant `karyon-git-auth` secretRefs stay (they work against public repos and survive any future posture change). (post-v0.20 quick task)
...
> **Drift note (2026-07-06, closed 2026-07-07):** the repo spent ~2 months LIVE-PRIVATE (unrecorded operator flip between Phase 9 and Plan 11-07). **Public visibility was restored 2026-07-07** after a clean strict-history gitleaks scan and PII/employer sweep. The per-tenant GitOps sources use `karyon-git-auth` secretRefs, which work under either posture.
