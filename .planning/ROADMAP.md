...
- [ ] **Phase 8: Capsule + capsule-proxy Bare-Minimum Install** — Two upstream HelmReleases reconciled hub-only into spoke-capsule; operator alive + proxy reachable + CRDs visible (plans 0-3 executed 2026-04-29; verifier `08-VERIFICATION.md` reports `gaps_found` 6/10 — 2 BLOCKER gaps G-03 cert source + G-04 architectural seam need fix-plans before close-out; the prior `[x]` mark in commits `5a94496` / `f693b5f` was a wave-tracking update applied before the verifier ran)
