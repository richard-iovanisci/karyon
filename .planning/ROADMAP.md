...
**Success Criteria** (what must be TRUE):
  ...
  3. A hand-crafted in-hub-pod probe from the `kustomize-controller` pod validates TLS with `openssl s_client -verify_return_error -verify_hostname k3d-<spoke>-server-0 -CAfile <embedded CA>` and then returns `ok` from `wget --no-check-certificate --header='Authorization: Bearer <TOKEN>' https://k3d-<spoke>-server-0:6443/readyz` for both spokes.
