...
- [x] **Phase 5: Workloads + Health + Rebuild** - podinfo + `nvidia-smi` pod + `task fix-dns` + `task health-check` + `task rebuild` < 20 min *(completed 2026-04-28)*
...
- [x] 03-06-PLAN.md — Gap closure: live/destructive bootstrap idempotency proof on real k3d-hub-flux cluster (FLUX-04; human-verification checkpoint)
...
**Risk**: HIGH — this is the single highest-risk phase in the project. DNS (`k3d-<spoke>-server-0` resolvable on `k8s-net`), TLS (`--tls-san` SAN present), CA-data (base64 PEM embedded correctly), RBAC (cluster-admin CRB), token format (legacy Secret-backed, NOT TokenRequest — `kubectl create token` expires), and Secret key name (`value.yaml`) all converge. P18 silent-misroute risk is a missing `spec.kubeConfig` block: controllers then fall back to the hub's in-cluster SA and reconcile into the WRONG cluster while reporting `Ready=True`; wrong/missing `secretRef.key` is loud on current Flux but remains pinned explicitly.
...
**Success Criteria** (what must be TRUE):
  ...
  3. A hand-crafted in-hub-pod probe validates TLS with `openssl s_client -verify_return_error -verify_hostname k3d-<spoke>-server-0 -CAfile <embedded CA>` and then sends the bearer-token `/readyz` and `/api/v1/nodes` requests over the same verified TLS connection via stdin (no bearer token in argv, no `--no-check-certificate`) for both spokes.
...
- [x] 05-05-PLAN.md — Destroy/rebuild wrappers + Taskfile wiring
- [x] 05-06-PLAN.md — Live destructive rebuild execution and timing verification
...
- [x] 06-02-PLAN.md — DOCS-07/08/09/10: ADRs 0002..0005 (k3d-over-Kind, Flux-over-ArgoCD, hub-only Flux, version pin)
- [x] 06-03-PLAN.md — DOCS-02 + DOCS-04: docs/architecture.md Mermaid topology + docs/gpu-notes.md
