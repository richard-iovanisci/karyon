...
| v0.19 phase-8 gap | G-03 (BL-01) — `certManager.generateCertificates: true` in `pocs/capsule/proxy/helmrelease.yaml:75-76` requires absent cert-manager controller; proxy pod cannot mount TLS volume | **Resolved 2026-04-30** (D-08-11 cert flip, commit `75e5b78`; row reconciled 2026-07-06 — see this file's Pending Todos entry) | Phase 8 verifier 2026-04-29 |
| v0.19 phase-8 gap | G-04 — outer `clusters/hub-flux/pocs/capsule.yaml` `spec.kubeConfig: spoke-capsule-kubeconfig` redirects HR/OCIRepo CRs to spoke which has no Flux CRDs; outer Kustomization Ready=False on push | **Resolved 2026-04-30** (D-08-12 split-Kustomization, commit `a9af679`; row reconciled 2026-07-06 — see this file's Pending Todos entry) | Phase 8 verifier 2026-04-29 |
| v0.19 close-out (still deferred into v0.20) | ADDON-01 / ADDON-02 capsule-addon-fluxcd Trial + hand-managed vs addon-managed comparison doc | **Resolved 2026-07-06: PASS (do not adopt)** per ADR-008 addendum — same-cluster kubeconfig factory, unusable under ADR-004; P27 already delivers the goal | v0.19 close 2026-05-18 |
...
| 2026-07-06 | tenant-ui-oidc | OIDC wiring LIVE (issuer https://localhost:31443, apiserver flags via k3s config.yaml retrofit, socat forwarder) + Headlamp v0.43.0 tenant UI through capsule-proxy (zero-RBAC, user-token passthrough, PKCE) + Tier-3 OIDC Group owners on tenants + headless e2e proof (alice=all tenants, bob=alpha only) + docs (architecture-flux-capsule-keycloak.md, tenant-access.md, draw.io diagram). See `.planning/quick/20260706-tenant-ui-oidc/` |
