...
must_haves:
  truths:
    - "DOCS-05 (D-04): docs/rebuild-runbook.md is laid out as linear numbered steps with timings on top, then a failure-mode appendix at the bottom."
    - "DOCS-05 (specifics): the runbook references the Phase 5 baseline of `rebuild elapsed seconds: 190` from /tmp/karyon-rebuild.log as the warm-cache SLO baseline, plus per-step timings (approximate from Phase 5 observations + the recorded total)."
    - "DOCS-05: the 7 step markers (`>>> step preflight`, `>>> step destroy`, `>>> step create-clusters`, `>>> step bootstrap-flux`, `>>> step register-spokes`, `>>> step deploy-examples`, `>>> step health-check`) appear in the runbook IN STRICT ORDER as the section structure."
    - "DOCS-05 (specifics): expected timings disclaim 'warm CUDA cache, RTX 5090 dev box' (per RESEARCH.md Open Question #5 and Assumption A4)."
    - "DOCS-05 (D-04): a `## Common failures` appendix at the bottom is keyed by symptom (stale CoreDNS NodeHosts → `task fix-dns`; missing GPU capacity → IMG-04 check; TLS-SAN mismatch → recreate the cluster; etc.)."
  artifacts:
    - path: "docs/rebuild-runbook.md"
      provides: "DOCS-05: linear rebuild procedure + per-step timings + failure-mode appendix"
      contains: ">>> step preflight"
      min_lines: 80
  key_links:
    - from: "docs/rebuild-runbook.md"
      to: "scripts/rebuild.sh (the >>> step markers source of truth)"
      via: "literal step-marker strings"
      pattern: ">>> step "
    - from: "docs/rebuild-runbook.md"
      to: "docs/architecture.md, docs/gpu-notes.md, docs/flux-hub-spoke.md, docs/wsl-networking.md"
      via: "cross-links in the failure-mode appendix"
      pattern: "docs/[a-z-]+\\.md"
---

<objective>
Author the `docs/rebuild-runbook.md` step-by-step procedure (D-04 layout):
linear numbered steps with expected wall-clock timings on top, then a
failure-mode appendix at the bottom keyed by symptom. The 7 step markers
(`>>> step preflight` … `>>> step health-check`) come verbatim from
`scripts/rebuild.sh:49-79` so the runbook headings stay aligned with the
runtime log markers — one source of truth.

Per RESEARCH.md Open Question #5: per-step timings are NOT in
/tmp/karyon-rebuild.log (only the total `rebuild elapsed seconds: 190` is
recorded). This plan does NOT re-time per-step (would require a destructive
rebuild run); instead, it cites the 190-second total as the SLO and
distributes approximate per-step timings from observed Phase 5 logs in the
git history (PROJECT.md and 05-CONTEXT.md). Approximate — the runbook
disclaims "warm cache, RTX 5090, 20-core dev box" baseline so future
contributors with different hardware understand the timings are
illustrative, not normative.

Purpose: DOCS-05. The runbook is the canonical mid-incident reference
("the lab is broken; what is the recovery procedure?"). It must be skimmable,
linear, and route readers to the right `task` invocation per failure
symptom.

Output: 1 NEW Markdown file under `docs/`; bats DOCS-05 contract green.
</objective>
...
<interfaces>
<!-- The 7 step markers — source of truth in scripts/rebuild.sh:49-79
(verified by tests/bats/destroy-rebuild-01-static.bats:129-146):
...
The runbook MUST use these literal strings as section anchors so a developer
mid-incident can grep `/tmp/karyon-rebuild.log` to see which step failed and
jump straight to that section in the runbook. -->

<!-- Phase 5 timing baseline (PROJECT.md + 05-CONTEXT.md + STATE.md):
  - Total: rebuild elapsed seconds: 190 (warm cache, RTX 5090 + 20-core)
  - Per-step distribution (approximate; reconstructed from Phase 5 SUMMARY notes):
      preflight:        ~5s   (read-only checks)
      destroy:         ~10s   (k3d cluster delete x3 + network rm)
      create-clusters: ~45s   (k3d cluster create x3 with image pull from local cache)
      bootstrap-flux:  ~30s   (flux bootstrap github + reconciliation handshake)
      register-spokes: ~30s   (SA + CRB + Secret + kubeconfig encode + 2 spokes)
      deploy-examples: ~30s   (Flux GitRepository sync + Kustomization apply on 2 spokes)
      health-check:    ~40s   (probes + DNS + TLS + workload validation)
      Total:           ~190s

These are illustrative warm-cache numbers; cold cache (rebuilding the CUDA
image from scratch) would add ~5-10 minutes to create-clusters. -->

<!-- Common failure-mode appendix per CONTEXT.md `<specifics>` 91-93 +
RESEARCH.md §Architecture Patterns §Failure Modes:
  - Stale CoreDNS NodeHosts → task fix-dns (HEALTH-07)
  - Missing GPU capacity → check IMG-03/IMG-04/IMG-05 (3-part contract)
  - TLS-SAN mismatch → recreate cluster (--tls-san is immutable post-create per CLU-05)
  - .env missing keys → preflight PRE-09 fails fast
  - Flux bootstrap PAT invalid → rotate via GitHub UI
  - Build cache evaporated → cold rebuild of CUDA image (~5-10 min surcharge)
-->
</interfaces>
</context>
...
<task type="auto">
  <name>Task 1: Author docs/rebuild-runbook.md with linear steps + per-step timings + failure appendix</name>
  <files>docs/rebuild-runbook.md</files>
  <read_first>
    - scripts/rebuild.sh full file (mandatory pre-read — the 7 step markers in lines 49-79 are the SOURCE OF TRUTH; runbook structure mirrors them)
    - tests/bats/destroy-rebuild-01-static.bats lines 129-146 (the exact ordering contract the runbook must satisfy via `tests/bats/docs-05-runbook.bats`)
    - .planning/PROJECT.md "Core Value" + "Validated" rows showing 190s warm-cache rebuild
    - .planning/phases/05-workloads-health-rebuild/05-CONTEXT.md (the rebuild's UX + log-recording decisions; per-step timing context)
    - .planning/STATE.md decision log lines 117-121 (Phase 5 final-verifier decisions)
    - .planning/phases/06-repo-hygiene-docs-adrs/06-CONTEXT.md `<decisions>` D-04 (linear-steps-then-appendix layout) + `<specifics>` 418-419 (190s baseline + warm-cache disclaimer)
    - .planning/phases/06-repo-hygiene-docs-adrs/06-RESEARCH.md Open Question #5 lines 1159-1171 (recommendation: approximate per-step from observation, disclaim warm-cache + dev-box baseline)
    - .planning/phases/06-repo-hygiene-docs-adrs/06-PATTERNS.md "docs/rebuild-runbook.md" section
    - docs/wsl-networking.md (analog: `**Step 1:**` numbered procedural style)
    - docs/flux-hub-spoke.md (analog: top-of-file h1 + orientation paragraph + table-style for structured rules)
  </read_first>
  <action>
Use `Write` to create `docs/rebuild-runbook.md`. Content (D-04 layout — linear
steps + timings + failure appendix):
...
```
  ...
  <verify>
    <automated>bats tests/bats/docs-05-runbook.bats</automated>
  </verify>
  <acceptance_criteria>
    - `[ -f docs/rebuild-runbook.md ]` true
    - `head -1 docs/rebuild-runbook.md` matches `^# ` (MD041)
    - All 7 step markers present and IN ORDER (ordered-literal grep): `>>> step preflight` < `>>> step destroy` < `>>> step create-clusters` < `>>> step bootstrap-flux` < `>>> step register-spokes` < `>>> step deploy-examples` < `>>> step health-check`
    - `grep -E '\b190\b' docs/rebuild-runbook.md` exits 0 (Phase 5 baseline reference)
    - `grep -iE '(seconds|minute)' docs/rebuild-runbook.md` exits 0 (timing units)
    - `grep -iE '^## (common failures|failure modes)' docs/rebuild-runbook.md` exits 0 (D-04 appendix heading)
    - Failure-mode keywords present: `grep -iF 'fix-dns' docs/rebuild-runbook.md` exits 0; `grep -iF 'nvidia.com/gpu' docs/rebuild-runbook.md` exits 0; `grep -iF 'tls-san' docs/rebuild-runbook.md` exits 0
    - `wc -l < docs/rebuild-runbook.md` returns >= 80
    - Cross-links to other docs present: `flux-hub-spoke.md`, `gpu-notes.md`, `wsl-networking.md`, `architecture.md` each grep with `-F` exit 0
    - `bats tests/bats/docs-05-runbook.bats` is fully green
  </acceptance_criteria>
  <done>docs/rebuild-runbook.md committed; bats `DOCS-05` tests fully green; runbook has 7 step markers in strict order, references the 190s baseline, has a Common failures appendix.</done>
</task>
...
This is a documentation-authoring plan; no application surface, no live services, no secret material. Pre-commit hook from Plan 01 gates the commit.

## STRIDE Threat Register

| Threat ID | Category | Component | Disposition | Mitigation Plan |
|-----------|----------|-----------|-------------|-----------------|
| (n/a) | (n/a) | rebuild-runbook.md | accept | The runbook references the GitHub PAT rotation procedure (failure-mode appendix entry "Flux bootstrap PAT invalid") but the prose contains no real PAT — it directs the reader to the GitHub UI. Plan 01 pre-commit hook would catch any accidentally embedded token. |
</threat_model>

<verification>
```bash
bats tests/bats/docs-05-runbook.bats     # all DOCS-05 tests green

grep -E '^## ' docs/rebuild-runbook.md   # expected: ~10 ## headings, with `## Common failures` near the end
```

Manual:
- Open `docs/rebuild-runbook.md` in a Markdown previewer; confirm linear flow
  (Pre-flight → 8 numbered steps → Total → Common failures → Reference) with
  proper headings and the 7 `>>> step <name>` literals visible.
- The "Common failures" appendix has clear symptom→recovery pairs for every
  documented failure mode.
</verification>

<success_criteria>
- 1 NEW file committed: docs/rebuild-runbook.md
- bats DOCS-05 tests fully green
- 7 step markers in strict order
- 190-second baseline referenced; warm-cache disclaimer present
- Common failures appendix at the bottom keyed by symptom
- All cross-links resolve (after Plans 01-05 land for the upstream artifacts)
- Plan committed
</success_criteria>

<output>
After completion, create `.planning/phases/06-repo-hygiene-docs-adrs/06-06-SUMMARY.md` recording:
- Runbook line count + section count
- Per-step timing breakdown captured
- bats output (DOCS-05 green)
- Note: per-step timings are reconstructed approximations from Phase 5 logs in git history; remeasure during a future rebuild and update if they drift > 25%.
</output>
