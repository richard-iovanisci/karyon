...
TAP_OUT="${KARYON_03_06_TAP_OUT:-/tmp/karyon-03-06-live.tap}"
ASSUME_YES=0
PRECHECK_ONLY=0
...
Options:
  ...
  --tap-out PATH     Write TAP output to PATH instead of /tmp/karyon-03-06-live.tap.
...
run_live_bats() {
  ...
  if grep -F "ok 8 FLUX-04 + D-13 (live/destructive, combined)" "$TAP_OUT" | grep -qF "# skip"; then
    fail "live/destructive test was skipped; expected it to run with both gates set"
  fi
  grep -F "ok 8 FLUX-04 + D-13 (live/destructive, combined)" "$TAP_OUT" >/dev/null \
    || fail "TAP output did not contain the required ok 8 live/destructive proof line"
...
print_supporting_evidence() {
  ...
  cat <<EOF
approved - TAP shows ok 8 (FLUX-04 + D-13 combined) and all preconditions passed
